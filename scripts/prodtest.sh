...
# Vault's credentials live outside ENV_FILE because ENV_FILE is regenerated on
# every run, while Vault's data volume is not.
...
if [[ "${1:-}" == "--down" ]]; then
    ...
    rm -f "${ENV_FILE}" "${UNSEAL_KEY_FILE}" "${ROOT_TOKEN_FILE}" \
          "${CREDENTIALS_FILE}"
...
# These four outlive a single run, because the state they protect outlives it
# too. Postgres fixes its password when the data volume is first initialised,
# so a fresh password on a surviving volume simply locks the gateway out. And
# PAN_KEY is worse than that: rotating it leaves every stored PAN undecipherable
# with no error at write time. Both are the same rule — a credential may only be
# regenerated together with the data it guards, which is what --down does.
if [[ -r "${CREDENTIALS_FILE}" ]]; then
    # shellcheck disable=SC1090
    source "${CREDENTIALS_FILE}"
    ok "Reusing the credentials from the earlier run (its volumes are still here)"
else
...
# float in the hot wallet, which is what actually stops a transfer.
AMOUNT_MAX=${AMOUNT_MAX:-3000.00}
...
# credentials from the first time are the only way back in. That is why both
...
# On disk rather than only in the env file, because the env file is rewritten
# wholesale at the top of every run — which would otherwise destroy the only
# copy of the token for a Vault that survives that same run.
umask 077
...
case "${STATUS}" in
    CRYPTO_SENT)
        # Which chain this settled on decides whether anyone else can check it.
        # A local Anvil is reachable from this machine and nowhere else, so
        # pointing at a public explorer would be an invitation to click a link
        # that shows nothing — worse than offering no link at all.
        if (( LOCAL_CHAIN )); then
            ok "Settled on the local chain."
            [[ -n "${HASH}" ]] && echo "     tx ${HASH}"
            echo "     No public explorer can see this: the chain runs in"
            echo "     gateway-anvil-prodtest and is not connected to anything."
            echo "     Verify it against the node itself:"
            echo "       scripts/verify_wallet.sh"
        else
            ok "Settled on a public testnet."
