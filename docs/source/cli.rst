...
.. code-block:: text
    ...
    optional arguments:
      ...
      --config-file CONFIG_FILE
                            File containing libfin configuration - JSON format
...
* set **LIBFIN_CONFIG** environment variable to point to folder containing ``libfin.json`` file

The format is a JSON object containing the config variable from the package config.py file.
See :py:mod:`libfin.config`.

.. warning::
    This is an example only. Please refer to :py:mod:`libfin.config` for full details.
