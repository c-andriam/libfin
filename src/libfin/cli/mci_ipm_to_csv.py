...
from libfin.cli import add_version, get_config, print_banner, print_exception_details
from libfin.mciipm import IpmReader, MciIpmDataError, ipm_info
...
def cli_run(**kwargs):
    ...
    config = get_config('libfin.json', cli_filename=kwargs.get('config_file'))
...
def cli_parser():
    ...
    parser.add_argument('--config-file', help='File containing libfin configuration - JSON format')
...
def mci_ipm_to_csv(in_ipm, out_csv, config, in_encoding=None, no1014blocking=False, **_):
    ...
    :param config: dict containing libfin config
