...
from libfin import iso8583, config, LibfinError, BitArray
from libfin.vendor import hexdump

LOGGER = logging.getLogger(__name__)


class MciIpmDataError(LibfinError):
    pass
...
class Unblock1014(object):
    ...
    def read(self, bytes_to_read: int = 0) -> bytes:
        """
        ...
        while read_all or len(self.buffer) <= bytes_to_read:
            ...
            if not block:  # eof
                break
            self.buffer.extend(block[:1012])

        if read_all or bytes_to_read >= len(self.buffer):
            output = bytes(self.buffer)
            self.buffer.clear()
        else:
            output = bytes(self.buffer[:bytes_to_read])
...
class VbsReader(object):
    ...
    ::
        ...
        from libfin.mciipm import VbsReader
...
class IpmReader(VbsReader):
    ...
    ::
        ...
        from libfin.mciipm import IpmReader
...
class IpmParamReader(VbsReader):
    ...
    ::
        ...
        from libfin.mciipm import IpmParamReader
...
class IpmWriter(VbsWriter):
    ...
    def write(self, obj: dict) -> None:
        ...
        See :py:mod:`libfin.iso8583` for expected dict object keys.
...
def vbs_list_to_bytes(byte_list: iter, **kwargs) -> bytes:
    ...
    :param kwargs: any options to be passed to VbsWriter constructor. See :py:mod:`libfin.mciipm.VbsWriter`
...
def vbs_bytes_to_list(vbs_bytes: bytes, **kwargs) -> list:
    ...
    :param kwargs: any options to be passed to VbsReader constructor. See :py:mod:`libfin.mciipm.VbsReader`
