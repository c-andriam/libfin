...
Import the library::
    ...
    from libfin.iso8583 import dumps, loads
...
from libfin import LibfinError
from libfin.BitArray import BitArray
from libfin.card import mask
...
class Iso8583DataError(LibfinError):
    pass
...
def dumps(obj: dict, encoding=None, iso_config=None, hex_bitmap=False):
    ...
    The default usage will generate a latin_1 encoded message with a binary bitmap::
        ...
        import libfin.iso8583
        message_dict = {'MTI': '1144', 'DE2': '4444555566667777'}
        libfin.iso8583.dumps(message_dict)
...
def loads(b: bytes, encoding=None, iso_config=None, hex_bitmap=False):
    ...
    ::
        ...
        libfin.iso8583.loads(message_bytes)
...
def _get_date_from_string(field_data: str) -> datetime.datetime:
    """
    Parse string dates to python datetime object

    Uses standard library fromisoformat which is fast and built-in.
    ...
    for date_format in date_formats:
        try:
            return datetime.datetime.strptime(field_data, date_format)
        except ValueError:
            continue
    raise ValueError(f"Unrecognised date string format - {field_data}")
