...
* :py:mod:`libfin.pinblock.Iso0PinBlock`
* :py:mod:`libfin.pinblock.Iso4PinBlock`
...
.. note:: The use of an encryption mix-in requires the install of additional modules.
          Use ``pip install libfin[crypto]``.

Available pinblock encryption mix-ins:

* :py:mod:`libfin.pinblock.TdesEncryptedPinBlockMixin`
* :py:mod:`libfin.pinblock.AesEncryptedPinBlockMixin`
...
* :py:mod:`libfin.pinblock.VisaPVVPinBlockMixin`
...
class TdesEncryptedPinBlockMixin(abc.ABC):
    ...
    def encrypt(key: str, data: bytes) -> bytes:
        ...
        if len(binary_key) == 8:
            binary_key = binary_key * 3
        elif len(binary_key) == 16:
            binary_key += binary_key[:8]
...
def calculate_pvv(pin: str, pvv_key: str, key_index: int, card_number: str):
    ...
    if len(bin_pvv_key) == 8:
        bin_pvv_key = bin_pvv_key * 3
    elif len(bin_pvv_key) == 16:
        bin_pvv_key += bin_pvv_key[:8]
